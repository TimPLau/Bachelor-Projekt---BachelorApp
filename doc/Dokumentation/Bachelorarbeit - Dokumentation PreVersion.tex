...
\par \bigskip Im weiteren Verlauf des Kapitels werden die grundsätzlichen Funktionsweisen der Bibliothek kurz beschrieben, um die im späteren Teil der Arbeit zur Nachvollziehbarkeit benötigten Kenntnisse zu ermöglichen.

\subsection{Komponenten einer Redux-Architektur}
...
\par \bigskip Die wichtigsten Redux-spezifischen Komponenten tragen die folgenden Bezeichnungen und werden in den nachfolgenden Kapiteln beschrieben:
...
\par \bigskip Da sich die Herausforderungen an dem Ablauf einer Bachelorarbeit orientieren und den Studierenden somit vor typische Aufgaben stellt, kam es im Verlauf der Interviews zu den Befürchtungen \ref{risk:realerZustand}, dass die Applikation nicht den realen Zustand der Bachelorarbeit kennt und somit der Studierende durch das Erfüllen von Herausforderungen ein Gefühl von Erfolg hat, obwohl diese Herausforderungen nicht den geforderten Qualitäten entsprechen und der Studierende dies durch das positive Feedback nicht merkt.
...
\par \bigskip Die Befürchtung \ref{risk:keinLerneffekt}, dass die Applikation den Studierenden so viel Arbeit abnimmt, dass sie zu wenig eigenständige Erfahrung sammeln und somit der Lerneffekt zu gering ist, wird berücksichtigt, indem die Applikation einen eher informierenden Charakter aufweist, und keine Funktionalität bezüglich dem Liefern von Ergebnissen vorsieht.

\par \bigskip Die Gefahren der Risiken \ref{risk:inhaltUnumstoesslich} und \ref{risk:erstazBetreuer}, welche sich damit auseinander setzen, dass die Applikation fälschlicherweise als Ersatz eines Betreuers wahrgenommen oder der Inhalt der Applikation als unumstößlich angesehen werden könnte, soll durch eine klare Kommunikation mit den Anwendern reduziert werden. Es werden als Maßnahmen einerseits immer die Quellen der Informationen angegeben und weiterhin eine allgemein sichtbare Erklärung zur Anwendung, sowie Fehlanwendung der Applikation deutlich kommuniziert.

\par \bigskip  Das identifizierte Risiko \ref{risk:ueberplanung}, welches die Befürchtung einer Überplanung der Studierenden beinhaltet, was die Auswirkung haben kann, dass die Studierenden von der eigentlichen Arbeit abgehalten werden, wird als nicht ausschlaggebend betrachtet.
...
\subsection{Reducertests}
\par Im Rahmen der Softwaretests wird speziell das Testen der Reducer Funktionen im Kontext der Redux-Architektur als essenziell betrachtet.
\par Bei den durchgeführten Tests wurde sich primär an den im Kontext der Redux-Architektur identifizierten Actions orientiert und eine systematisch aufgebaute Abfolge von Tests, welche das Überprüfen des Systemzustands vor sowie nach Ausführen einer Action vorsieht.
\par \bigskip Insgesamt wurden TODO Reducer Testeinheiten ausgeführt, welche sich alle als positiv abgeschlossene Stichproben herausstellen lassen.
\par \medskip Die folgenden Testklassen wurden im Rahmen der Reducer Tests definiert: 

\section{Ergebnisse der Nutzerevaluation}
TODO AUF ANANG HINWEISEN UND ANHANG EINFÜGEN
...
\par Unter den 5 Studierenden befinden sich genau 3 Studierende in der Phase während der Bachelorarbeit und 2 Studierende am Ende des 5. Semesters vor der Bearbeitung der Bachelorarbeit.

\par \bigskip Der Test wurde unter Moderation durch Stellen verschiedener vordefinierter Aufgaben durchgeführt, dabei wurde aufgrund der vergleichbar zu haltenden gesammelten Erfahrung und Beurteilung des Tests, nicht zwischen Professoren oder Studierenden unterschieden, sondern der jeweils gleiche Testablauf gewählt.

\par \medskip Das Bildschirmgeschehen aller Tests wurden mit Hilfe einer Software auf dem eigenen Smartphone aufgezeichnet, jedoch ist eine Datei dieser Aufzeichnungen durch einen Datenfehler nicht verwendbar.

\par \medskip Des weiteren wurden nach vorher eingeholter Erlaubnis der Testpersonen, ein Audiomitschnitt erzeugt. Dieser Audiomitschnitt wurde von beiden Professoren und insgesamt 4 von 5 Studierenden zugelassen.

\par \medskip Der in Form von nacheinander gestellten Aufgaben verwendete Testablauf ist im Anhang zu finden und Umfasst 12 Aufgaben (siehe TODO ANHANG).

\subsection{Zusammenfassung der Testdurchführung}
\par Zusammenfassend hat sich ergeben, dass alle Testpersonen die vorgegebenen Aufgaben fast ohne Schwierigkeiten lösen konnten und der Großteil der Aufgaben ohne Hilfestellung des Moderators erfüllt wurden. Falls ein Eingriff des Moderators erforderlich wurde, so hatte dies lediglich den Zweck, den Fokus der Testpersonen wieder auf die zu lösende Aufgabe zu lenken.
\par Falls Irritationen bezüglich des Lösens der Aufgaben bei den Testpersonen aufgetreten sind, so konnten diese Selbstständig beseitigt werden.

\par \bigskip Im folgenden Verlauf werden die Ergebnisse des Tests durch verschiedene definierte Parameter, kategorisiert nach den jeweiligen Aufgabenschwerpunkten, zusammengefasst dargestellt. Weiterhin werden, falls vorhanden, die Kernaussagen der aufgabenspezifischen Kommentare und Wünsche der Testpersonen beschrieben.

\begin{itemize}
\item Kategorie 1 - Der Startbildschirm bei erstmaligem Ausführen der Applikation
\par 

\item Kategorie 2 - Bachelorarbeit Guide
\par

\item Kategorie 3 - Meilensteinverwaltung
\par

\item Kategorie 4 - Achievements
\par

\item Kategorie 6 - Herausforderungen
\par

\item Kategorie 5 - Dashboard
\par

\end{itemize}

\subsection{Zusammenfassung der Befragung von Studierenden}


\section{Bewertung des Frameworks Flutter zur Entwicklung von mobilen Appliaktionen}
\par Im Laufe der Entwicklung der Applikation haben sich sowohl subjektive Erfahrung als auch objektive zu bewertende Fakten in Bezug auf die Nutzung von Flutter zur Entwicklung mobiler Applikationen herausgestellt, welche im folgenden Verlauf diskutiert und abschließend durch eine Gegenüberstellung der jeweiligen Vor- und Nachteile ausgewertet werden.
